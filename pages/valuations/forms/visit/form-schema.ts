...
const visitFormSchema = z.object({
    ...
    project_start_date: undefinedOrEmptyToNull,
    project_end_date: undefinedOrEmptyToNull,
    ...
    return !(isConstructionCompleted(data.construction_stage) && isNullOrEmpty(data.construction_year))
}, {
    message: 'Construction Year Required',
    path: ['construction_year']
}).refine((data) => {
    return !(isConstructionCompleted(data.construction_stage) && data.construction_year != null && Number(data.construction_year) < 1800)
}, {
    message: 'Construction Year Must Be Greater Than 1799',
    path: ['construction_year']
}).refine((data) => {
    return !(isConstructionCompleted(data.construction_stage) && isNullOrEmpty(data.property_type))
}, {
    message: 'Property Type Required',
    path: ['property_type'],
}).refine((data) => {
    if (isConstructionCompleted(data.construction_stage) && isNullOrEmpty(data.structure_type)) {
        return false
    }
    return true
}, {
    ...
    if (isConstructionCompleted(data.construction_stage) && isNullOrEmpty(data.property_condition)) {
        return false
    }
    return true
}, {
    ...
    return !(data.construction_year !== null && data.construction_year !== '' && Number(data.construction_year) > new Date().getFullYear())
}, {
    message: `Construction Year Must Be Less Than Or Equal To ${new Date().getFullYear()}`,
    path: ['construction_year'],
}).refine((data) => {
    return !(isUnderConstruction(data.construction_stage) && isNullOrEmpty(data.project_start_date))
}, {
    message: 'Project Start Date Required',
    path: ['project_start_date'],
}).refine((data) => {
    return !(isUnderConstruction(data.construction_stage) && isNullOrEmpty(data.project_end_date))
}, {
    message: 'Project End Date Required',
    path: ['project_end_date'],
}).refine((data) => {
    return !(data.project_start_date != null && data.project_end_date != null && data.project_start_date > data.project_end_date)
}, {
    message: 'Project End Date Must Be Later Than Project Start Date',
    path: ['project_end_date'],
}).refine((data) => {
    return !(isUnderConstruction(data.construction_stage) && isNullOrEmpty(data.work_completed))
}, {
    message: 'Construction Details Work Completed Required',
    path: ['work_completed'],
}).refine((data) => {
    return !(isLocalityNegative(data.locality_negative) && isNullOrEmpty(data.locality_negative_remark))
}, {
...
const visitOccupancyFormSchema = z.object({
    id: z.number().optional(),
